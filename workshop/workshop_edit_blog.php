...
if (isset($_POST['title']) && isset($_POST['auth']) && isset($_POST['content'])){
  ...
  if (isset($_GET['id'])) {
    ...
    $bdd->query("UPDATE blog SET title=$title, auth=$auth, content=$content WHERE id=$id ");
  }else {
    $bdd->query("INSERT INTO blog SET title=$title, auth=$auth, content=$content, date=NOW() ");
  }
   
  setFlash('L\'article a bien été ajouté');
  header('Location: workshop_gestion_blog.php');
  die();
}

if (isset($_GET['id'])) {
  $id = $bdd->quote($_GET['id']);
  $select = $bdd->query("SELECT * FROM blog WHERE id=$id");
  ...
  <div class="row">
    <div class="container">
      <div class="col-sm-offset-1 col-sm-10">
        <a href="<?php echo WEBROOT; ?>workshop/workshop_gestion_blog.php"><i class="glyphicon glyphicon-chevron-left" id="glyph"></i>Retour</a>
